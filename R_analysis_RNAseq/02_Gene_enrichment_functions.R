...
run_overrepresentation_analysis <- function(gene_set, dds_res, 
  ...
  if(dim(msig_h.oa@result)[1] > 0){
      ...
      msig_h.oa.gs <- setReadable(msig_h.oa, org.Mm.eg.db, keyType = "ENSEMBL")
...
run_enrichment_analysis <- function(gene_set, geneList, 
  ...
  if(type == "reactome"){
    # Replace ensembl IDs by Entrez IDs for compatibility with gsePathway
    entrez_ids.df <- clusterProfiler::bitr(geneID = names(geneList), fromType = "ENSEMBL", toType = "ENTREZID", OrgDb="org.Mm.eg.db")
    geneList.entrez <- geneList[c(entrez_ids.df$ENSEMBL)]
    names(geneList.entrez) <- entrez_ids.df$ENTREZID
    msig_h.gsea <- gsePathway(geneList = geneList.entrez, organism = "mouse", pvalueCutoff = adj_p_cutoff, verbose = TRUE, eps = 0)
    if(dim(msig_h.gsea@result)[1] > 0){ 
      # Convert Entrez gene IDs to gene symbols
      msig_h.gsea.gs <- setReadable(msig_h.gsea, org.Mm.eg.db, keyType = "ENTREZID")
    ...
    if(dim(msig_h.gsea@result)[1] > 0){ 
      # Convert Entrez gene IDs to gene symbols
      msig_h.gsea.gs <- setReadable(msig_h.gsea, org.Mm.eg.db, keyType = "ENSEMBL")
      if(no_plots){return(msig_h.gsea.gs)}
    }  
  } 
...
go_gsea <- function(dds_res, my_file, golevel = 3, 
  ...
  go.gsea.gs <- setReadable(go.gsea , org.Mm.eg.db, keyType = "ENTREZID")
